...
//		return $queryuap->row_array();
	...
	public function update (){
		...
		$donasi_ap = number_format($jumlahdid['donasi_ap'],0);
		$donasi_aksi = number_format($jumlahdid['donasi_aksi'],0);
		$totaldonaksi = number_format($jumlahdid['totaldon'],0);		
		//email to donatur :
	                                                    ...
	                                                    <tr>
	                                                        <td valign='top' class='bodyContent'>
	                                                            <div mc:edit='std_content00'>
	                                                                ...
	                                                                Selamat, Aksi anda ($namaaksi) Telah mendapatkan donasi dari $pangdon, berikut data informasinya :
	                                                            </div>
															</td>
	                                                    </tr>
	                                                    <tr>
	...
	$this->email->cc('gufron@ayopeduli.com');
    $this->email->subject('Infomasi donasi untuk aksi anda di ayopeduli.com');
    $this->email->message($email_body);
	...
	public function addadmin(){
		if($_POST['action'] == 'insert'){ 
			...
			$jumlahis = number_format($jumlah,0);
			...
			//var_dump($datadon);					
			$this->db->insert('ap_donasi', $datadon); 
			//var_dump($terkumpul);
			$total = $jumlah + $terkumpul;
			$totalis = number_format($total,0);
	                                                ...
	                                                <table border='0' cellpadding='20' cellspacing='0' width='100%'>
	                                                    <tr>
	                                                        <td valign='top' class='bodyContent'>
	                                                            <div mc:edit='std_content00'>
	                                                                <h4 class='h4'>Hello $pangfas ,</h4>
	                                                                Selamat, Aksi anda ($namaaksi) Telah mendapatkan donasi dari $dari, berikut data informasinya :
	                                                            </div>
															</td>
	                                                    </tr>
	                                                    <tr>
	...
	?>		
		<div class='alert alert-success'>
            <button type="button" class="close" data-dismiss="alert">×</button> Terima kasih <?php echo $dari; ?> , Info Donasi anda telah diupdate</div>
            <script type="text/javascript">
            $(function(){
				$('#dari').val('');
				$('#jumlah').val('');
	...
	public function nonlog(){
		...
		if (count($email2)>0){	
			...
			<div class='alert alert-error'>
			  <button type="button" class="close" data-dismiss="alert">×</button>Email telah terdaftar silahkan login</div>
			  <script type="text/javascript">
				$(function(){
					$('#loading').show();					   
				});				
...
//			$this->load->view('footer');
	                                                ...
	                                                <table border='0' cellpadding='20' cellspacing='0' width='100%'>
	                                                    ...
	                                                    <tr>
	                                                      <td valign='top' class='bodyContent'>
                                                               ...
                                                               <strong>1. BRI Acc. 0139.01.002011.30.7 a.n Yayasan Ayo Peduli Nusantara KCP Bekasi - Juanda</strong><br /><br />
                                                               Note : <br />
                                                               1. Silahkan konfirmasi donasi melalui sms/wa ke no 0813 1570 4058 <br />
	...
	public function donlogin(){
       ...
       // $panggilan = mysql_real_escape_string($_POST['panggilan']);
		...
		$uid = mysql_real_escape_string($_POST['uid']);	
		$namaaksi = mysql_real_escape_string($_POST['namaaksi']);
		$sembunyi = mysql_real_escape_string($_POST['sembunyi']);	
		$donasi_ap = $totaldon - $donasi_aksi;	
		//$donasi_ap  = mysql_real_escape_string($_POST['donasi_ap']);		
		$aid  = mysql_real_escape_string($_POST['aid']);
		$now1 = date("m/d/Y H:i:s");
		...
		$did = autonumber("ap_donasi","did",5,"AP");
		$sqldonasi   = "insert into ap_donasi (donasi_aksi, did, uid, donasi_ap, time1, statdon, totaldon, aid, namaaksi, donatur, poin, sembunyi ) values ('$donasi_aksi', '$did', '$uid', '$donasi_ap', '$now1', 0, '$totaldon', '$aid','$namaaksi', '$fullname', '$poin', '$sembunyi')";
        $query = mysql_query($sqldonasi);
		$this->db->select('*');
		$this->db->from('ap_user');
		$this->db->where(array('uid' =>$uid));
	                                                ...
	                                                <table border='0' cellpadding='20' cellspacing='0' width='100%'>
	                                                    ...
	                                                    <tr>
	                                                      <td valign='top' class='bodyContent'>
                                                               ...
                                                               <strong>1. BRI Acc. 0139.01.002011.30.7 a.n Yayasan Ayo Peduli Nusantara KCP Bekasi - Juanda</strong><br /><br />
                                                               Note : <br />
                                                               1. Silahkan konfirmasi donasi melalui sms/wa ke Irma no 0813 1570 4058 <br />
															   <strong>Id Donasi # Nama # Tujuan Donasi # Jumlah Donasi</strong>                  
	                                                            </div>
														</td>
                                                    </tr>
...
public function getmydonasi($uid){
		$this->db->select_sum('totaldon');
		$query = $this->db->get_where('ap_donasi', array('uid' => $uid,'statdon' => '1'));
		return $query->row_array();	
}
public function getmyjumdon($uid){
	$this->db->select('*');
	$this->db->from('ap_donasi');
	$this->db->where(array('uid' => $uid, 'statdon' => '1'));
	$query = $this->db->get();
	return $query->result_array();
}
};
