...
//		return $queryuap->row_array();		
	...
	public function getvolunuid($uid2, $aid2){
		...
		$this->db->select('*');
		$this->db->from('ap_volunteer');
		$this->db->where(array('uidv' => $uid2,'aidv' => $aid2));
	...
	public function getvolunid($uid){
		...
		$this->db->join('ap_aksi', 'ap_volunteer.void = ap_aksi.aid');
		$query = $this->db->get();
		return $query->result_array();	
	}
	...
	public function insert (){
		...
		$this->db->select('*');
		$this->db->from('ap_volunteer');
		$this->db->where(array('uidv' => $uid,'aidv' => $aid));
		$query23 = $this->db->get();
		...
		if (count($uidsame)>0){
						...
						<?php }else {
			$sqlvolun   = "insert into ap_volunteer (void, uidv, aidv, date ) values ('$void', '$uid', '$aid', '$now1' )";
