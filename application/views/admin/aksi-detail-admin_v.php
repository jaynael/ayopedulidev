                                      ...
                                      <script type="text/javascript">
										$(function(){
											$('#loading').show();															
													...
													//	alert('clicked');
													var aid = $('#aidval').val();
													$.post('/atur/aksivalidasi',{action: "update", aid:aid},function(res){
													$('#result').html(res);
													});
												//});
												...
												//show records
												$('#show').click(function(){
													$.post('/atur/aksivalidasi',{action: "show"},function(res){
														$('#result').html(res);
													});        
												});
										});
                                       ...
                                       <input type="hidden" name="aidval" id="aidval" value="<?php echo $aksi_item['aid'] ?>" />
                                       <button name="validasi" id="validasi" class="btn btn-primary">Validasi Sekarang</button>
									<?php }
									else{ ?>
                    ...
                    <?php for ($i=0;$i<count($aksi_donasi_item);$i++){ ?>
                      <tr>
						...
						//var_dump($aksi_donasi_item[$i]['sembunyi']);
						 if ($aksi_donasi_item[$i]['sembunyi'] == 0 ){ 
					...
					<div class="form-group">
                            ...
                            <div class="col-sm-10">
                              <div class="input-prepend input-append">
                              		<div class="inp">
									<span class="add-on">Rp</span><input id="appendedPrependedInput" class="donasi_aksi required" name="donasi_aksi" size="16" type="text" onclick="var n=Math.random()*1000;document.getElementById('totaldon').value= Number(document.getElementById('appendedPrependedInput').value)+Number(n.toFixed(0));">
                                    </div>                                    
								</div>                               
                            </div>
                             <span>Jangan menggunakan koma atau titik Ex.500000</span>
                          </div>                          
                          <div class="form-group">
...
//									$('#result').html(res);
//								});
								$.post('/donasi/nonlog',{action: "insert", email:email, password:password, fullname:fullname, panggilan:panggilan,  donasi_aksi:donasi_aksi, donasi_ap:donasi_ap, totaldon:totaldon, aid:aid, photoimg:photoimg, namaaksi:namaaksi},function(res){
								$('#result').html(res);
								});
							//});
              ...
              <div class="tab-pane active" id="diskusi">
                    <div  style="margin:10px 0px">
                        ...
                        <fb:comments href="http://new.ayopeduli.com/aksi/view/<?php echo $aksi_item['slug'] ?>" width="600px" numposts="20" colorscheme="light"></fb:comments>
                    </div>
                </div>
            </div>
